Shader "GoHDR/Reflective/Bumped Unlit" {
	Properties {
		...
		SubShader {
			...
			Pass {
				...
				fixed4 frag (v2f i) : COLOR
				{
					...
					fixed4 c = LLDecodeGamma( UNITY_LIGHTMODEL_AMBIENT * 1.47 ) * texcol;
					c.rgb *= 2;
					fixed4 reflcolor = LLDecodeTex( texCUBE(_Cube, r) ) * LLDecodeGamma( _ReflectColor ) * texcol.a;
					
					return LLEncodeGamma( GoHDRApplyCorrection( c + reflcolor ) );
				}
