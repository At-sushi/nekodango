	...
	SubShader {
		...
		Pass {
			...
			v2f vert (appdata_full v) {	
				...
				o.color = LLDecodeGamma( v.color.a );
				return o;
			}
			...
			fixed4 frag (v2f i) : COLOR {
				fixed4 col = LLDecodeTex( tex2D (_MainTex, i.uv) );
				clip (col.a - _Cutoff);
				...
				half3 light = LLDecodeGamma( UNITY_LIGHTMODEL_AMBIENT * 1.47 ) * albedo;
				...
				for (int j = 0; j < 3; j++)
				{
					...
					half3 translucencyColor = backContribs[j] * LLDecodeGamma( _TranslucencyColor );
