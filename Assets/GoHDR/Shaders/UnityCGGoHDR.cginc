...
float3 ShadeVertexLights (float4 vertex, float3 normal)
{
	...
	#if defined(GAMMA_2_0) || defined(GAMMA_2_2)
	float3 lightColor = LLDecodeGamma( UNITY_LIGHTMODEL_AMBIENT * 1.47 );
	#else
...
inline fixed4 VertexLight( v2f_vertex_lit i, sampler2D mainTex )
{
	...
	texcol.rgb = LLDecodeGamma( texcol.rgb );
	fixed4 c;
